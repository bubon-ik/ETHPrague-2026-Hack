/**
 * Minimal demo: string / JSON / binary, keys(), entries(), delete().
 *
 * From swarm-kv root:
 *   npm install && npm run build
 *   cd examples && npm install && node basic.mjs
 *
...
if (!privateKey || privateKey.length < 66) {
  ...
  console.warn(
    "No PRIVATE_KEY in .env — generated one-off key (store not reopenable without saving it):",
    privateKey,
  );
}
...
console.log(
  'after delete("greeting"), has:',
  await kv.has("greeting"),
  "get:",
